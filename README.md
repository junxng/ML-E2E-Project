...
SECURITY_GROUP_ID=$(aws ec2 describe-instances --instance-ids YOUR_INSTANCE_ID --query 'Reservations[0].Instances[0].SecurityGroups[0].GroupId' --output text)
...
## Create or Check Virtual Environment & Activate it
```sh
...
```sh
aws s3 mb s3://kubeflow-bucket-dungnq49 --profile kubeflow-user
```
